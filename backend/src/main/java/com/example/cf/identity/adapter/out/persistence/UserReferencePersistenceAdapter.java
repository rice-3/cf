...
public class UserReferencePersistenceAdapter implements UserReferenceQuery {
    ...
    public String findEmail(String userId) {
        return jdbcTemplate.query("select email from app_user where user_id = ?", (rs, rowNum) -> rs.getString("email"), userId).stream()
                .findFirst().orElse(null);
    }

    @Override
    public boolean isActive(String userId) {
        Integer count = jdbcTemplate.queryForObject("select count(*) from app_user where user_id = ? and status = 'ACTIVE'", Integer.class,
                userId);
