...
public class AppUserRepository implements AppUserPort {
    ...
    private static final RowMapper<AppUserRecord> ROW_MAPPER = (rs, rowNum) -> new AppUserRecord(
            rs.getString("user_id"),
            rs.getString("cognito_subject"),
            rs.getString("email"),
    ...
    public Optional<AppUserRecord> findById(String userId) {
        return jdbcTemplate.query(
                "select * from app_user where user_id = ?", ROW_MAPPER, userId).stream().findFirst();
    }

    @Override
    public Optional<AppUserRecord> findByCognitoSubject(String cognitoSubject) {
        return jdbcTemplate.query(
                "select * from app_user where cognito_subject = ?", ROW_MAPPER, cognitoSubject)
                .stream().findFirst();
    }
    ...
    public boolean existsByEmailExcluding(String email, String excludeUserId) {
        Integer count = jdbcTemplate.queryForObject(
                "select count(*) from app_user where lower(email) = lower(?) and user_id <> ?",
    ...
    public int updateStatus(String userId, String status, long expectedVersion, Instant now) {
        return jdbcTemplate.update(
                "update app_user set status = ?, version = version + 1, updated_at = ? where user_id = ? and version = ?",
                status, Timestamp.from(now), userId, expectedVersion);
    }

    @Override
    public int touchVersion(String userId, long expectedVersion, Instant now) {
        return jdbcTemplate.update(
                "update app_user set version = version + 1, updated_at = ? where user_id = ? and version = ?",
    ...
    public SearchResult search(String keyword, String status, int page, int size) {
        ...
        Long total = jdbcTemplate.queryForObject(
                "select count(*) from app_user" + where, Long.class, args.toArray());

        List<Object> pageArgs = new java.util.ArrayList<>(args);
        pageArgs.add(size);
        pageArgs.add(page * size);
        List<AppUserRecord> items = jdbcTemplate.query(
                "select * from app_user" + where + " order by created_at desc limit ? offset ?",
