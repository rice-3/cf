...
- 対象リポジトリ: `F:\11\CF`
- 上位文書: 要件定義・技術選定 / 基本設計 BD-CF-001 / 詳細設計 DD-CF-001（`docs/des/`。`G:\マイドライブ\CF\` にも同期）
- 作成日: 2026-07-20 / 最終更新: 2026-07-28（工程10完了・フロントエンド全画面完了時点）
...
| 9 | Identity / Admin / Audit（API公開） | ✅ 完了（Cognito連携含む。JIT自動登録は ADR-0007 で承認済み） |
| 10 | 監視、CI/CD、E2E、運用手順 | ✅ 完了（実AWSへの apply と監視の実配線のみ残。`ses_ai_ddd_remaining_tasks.md` §2.1） |

第1段階（1〜4）に加え、工程5〜10がすべて完了。**フロントエンドも全19画面が完了**している。

残るのは実AWS環境での作業（apply・疎通確認・監視の実配線・SES登録）と、予算・上流待ちの項目のみ。
最新の残タスクは `ses_ai_ddd_remaining_tasks.md`、発見した問題と対応の記録は
`ses_ai_ddd_problem_catalog.md` を参照。
...
設計書は両書とも版数1.3（2026-07-28 に実装との突き合わせを実施）。`.docx` も同日 pandoc で再出力済み。所在は `docs/des/`。
...
- [ ] バッチ（基本設計 §8.1 の番号）: BAT-001 公開開始 / BAT-002 募集終了 / BAT-003 返金対象作成（`ProjectFailed` を購読） / BAT-004 返金実行 / BAT-005 通知送信 / BAT-007 決済照合（API-PY-002含む） / BAT-008 ファイル清掃 / BAT-009 監査アーカイブ
  ...
  - 冪等記録の削除は **BAT-010** として実装済み。基本設計 §8.1 にも 1.3 で追加した
...
- [x] git初回コミット・push 済み（`main`）
- [x] BFF配置 / 決済非同期UI / 本文プレーンテキスト は **ADR-0004 / ADR-0005 / ADR-0006** として起票済み
- [x] Outbox配送は **アプリ内配送で確定**（ADR-0008）。未使用のSQSキュー・IAM権限・VPCエンドポイントはTerraformから削除済み
