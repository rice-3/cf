...
| 9 | Identity / Admin / Audit（API公開） | ⬜ 未着手（監査ログ記録・UserReferenceQueryのみ） |
| 10 | 監視、CI/CD、E2E、運用手順 | ⬜ 未着手 |

第1段階（1〜4）に加え、第2段階の工程5〜7が完了。残るは工程8〜10とフロントエンド。
...
### 4.2 実装順序 9: Identity / Admin / Audit

- [ ] API-US-001/002 プロフィール取得・更新
- [ ] API-AD-001〜003 会員検索・ロール更新・会員停止
- [ ] API-AU-001 監査ログ検索 / API-AU-002 AI利用記録検索（テーブルは0004で作成済み、API未実装）
- [ ] Cognito（OIDC）認証への切替（dev以上、現状はlocalヘッダー認証のみ）
...
| 状態 | API |
|---|---|
| ✅ 実装済み | API-PJ-001〜006, API-RV-001〜006, API-FL-001〜002, API-FD-001〜004, API-PY-001〜002, API-RF-001〜002 |
| ⬜ 未実装 | API-US-001〜002, API-AD-001〜003, API-AU-001〜002 |
...
- 統合テスト6クラス: プロジェクト審査フロー / ファイルアップロード / 支援申込 / 決済Webhook・Outbox配送 / 工程8バッチ一気通貫 / 運用操作API
